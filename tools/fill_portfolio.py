...
def main():
    ...
    ap.add_argument("--capture-env", action="store_true",
                    help="rewrite benchmarks/ENVIRONMENT.md from this machine's nvidia-smi and ollama list")
    args = ap.parse_args()
    ...
    # run of this script on any other machine must not overwrite it.
    if not args.capture_env:
        print("  ENVIRONMENT.md left as is (pass --capture-env on the benchmark machine to rewrite it)")
    else:
        gpu = run(["nvidia-smi", "--query-gpu=name,memory.total,driver_version",
                   "--format=csv,noheader"])
        ollama = run(["ollama", "list"])
        env_lines = ["# Measured Environment", "",
                     "*Captured on the machine the benchmarks in the writeups ran on.*", ""]
